-- Safer partial/repeated refund workflow for the Flutter order manager.
...
begin
  ...
  from (
    ...
    left join (
      select ri.order_item_id,sum(ri.quantity) as refunded_quantity
    ...
    left join (
      select odi.order_item_id,sum(odi.discount_amount) as allocated_discount
...
begin
  ...
  return public.process_refund(
    p_order_id,p_items,v_returns,p_reason,auth.uid()
  );
end;
$$;
